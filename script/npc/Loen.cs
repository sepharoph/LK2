...
namespace LKCamelot.script.npc
{
    public class Loen : BaseNPC
    {
        ...
        List<script.item.Item> templ = new List<script.item.Item>()
            {
                new script.item.potions.LifeDrug(1),
                new script.item.potions.MagicDrug(1),
                new script.item.potions.FullLifeDrug(1),
                new script.item.potions.FullMagicDrug(1),
        ...
        public override void Buy(model.Player player, int buyslot)
        {
            if (player.GetFreeSlot() != -1 && player.Gold >= templ[buyslot].BuyPrice)
            {
                ...
                if (buyslot == 0)
                {
                    tempitem = new script.item.potions.LifeDrug().Inventory(player);
                    player.client.SendPacket(new LKCamelot.model.AddItemToInventory2(
                        tempitem).Compile());
                }
                if (buyslot == 1)
                {
                    tempitem = new script.item.potions.MagicDrug().Inventory(player);
                    player.client.SendPacket(new LKCamelot.model.AddItemToInventory2(
                       tempitem).Compile());
                }
                if (buyslot == 2)
                {
                    tempitem = new script.item.potions.FullLifeDrug().Inventory(player);
                    player.client.SendPacket(new LKCamelot.model.AddItemToInventory2(
                       tempitem).Compile());
                }
                if (buyslot == 3)
                {
                    tempitem = new script.item.potions.FullMagicDrug().Inventory(player);
                    player.client.SendPacket(new LKCamelot.model.AddItemToInventory2(
                       tempitem).Compile());
                }
